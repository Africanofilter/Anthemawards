...
**Live site:** [africanofilter.github.io/Anthemawards](https://africanofilter.github.io/Anthemawards/)

## What's in this repo

```
.
├── index.html          # the entire page (self-contained HTML/CSS)
...
No build step, no dependencies, no framework, and no JavaScript — just plain HTML and CSS. The "Our Approach" section uses the browser's native `<details>`/`<summary>` expand-collapse elements, so there's nothing to break across browsers or devices.
...
- **The "Our Approach" accordion**: each phase is a `<details class="phase-item">` block. To make a phase open by default, add the `open` attribute to its `<details>` tag (only one currently has it). No JavaScript is involved — the expand/collapse behavior is native to the `<details>` element.
